...
First it is needed to install ROS kinetic by following the instructions found [here](http://wiki.ros.org/kinetic/Installation). We used Ubuntu Mate on a Raspberry Pi 3 and followed the instructions for the Ubuntu install.
...
Clone this repository into your `/catkin_ws/src`. Next make your workspace by running this command from the `/catkin_ws/` directory:

```bash
$ rosmake
```

Connect your Raspberry Pi to your Parrot Sumo WiFi hotspot. (Note that the Sumo has a setting for both 2.4 and 5.8 Ghz broadcast. To change between the two, just push the power button once while the Sumo is on.) Plug in the controller to the Raspberry Pi and run this command:

```bash
$ roslaunch rossumo final.launch
```

Control Mapping
---------------

We used a Logitech Gamepad F310. The D pad controlled normal motion, the right trigger does a rapid 180 degree turn, the right bumper switches between 3 modes of operation (flip on back, balance on nose and normal operation) and the left bumper will cause the Sumo to jump vertically. 




Original documentation found below and [here](https://github.com/arnaud-ramey/rossumo). Note we did make changes to the original.
